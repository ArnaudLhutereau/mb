...
nb_thread = 0
# Stats
fichier = open("save_benchmark_store_in_blockchain.txt", "w")
...
while(cpt < number_file):
    ...
    while(nb_thread > (max_thread-1)):
        time.sleep(0.1)
    nb_thread = nb_thread + 1
    thread_bloc3 = TransactionBlockchainMetablock(meta_bloc_4)
    thread_bloc3.start()

    while(nb_thread > (max_thread-1)):
        time.sleep(0.1)
    nb_thread = nb_thread + 1
    thread_bloc3 = TransactionBlockchainMetablock(meta_bloc_5)
    thread_bloc3.start()

    while(nb_thread > (max_thread-1)):
        time.sleep(0.1)
    nb_thread = nb_thread + 1
    thread_bloc3 = TransactionBlockchainMetablock(meta_bloc_6)
    thread_bloc3.start()

    while(nb_thread > (max_thread-1)):
        time.sleep(0.1)
    nb_thread = nb_thread + 1
    thread_doc = TransactionBlockchainMetadoc(meta_doc)
    thread_doc.start()
