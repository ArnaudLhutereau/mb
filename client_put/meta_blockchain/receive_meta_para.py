...
store_var_contract = w3.eth.contract(
   address="0x97e72093A02Ebc3515d18B430213C1847060A5ED",
...
class TransactionBlockchainMetadoc(Thread):
    ...
    def run(self):
        ...
        ip_address_stats_container="172.18.0.4"
        link="http://"+ip_address_stats_container+":80/api/receive.php?request="+self.body['path']+"///"+str(time_redis[0])+"."+str(time_redis[1])+"///Start"
        requests.get(link)
        print("Thread started for metadoc ", self.body['path'], "currently time in redis: ", time_redis)
        
        tx_hash = store_var_contract.functions.addMetadoc(self.body['path'], self.body['creation_date'], self.body['blocks'], self.body['original_size']).transact()
        ...
        link="http://"+ip_address_stats_container+":80/api/receive.php?request="+self.body['path']+"///"+str(time_redis[0])+"."+str(time_redis[1])+"///Finish"
        requests.get(link)
class TransactionBlockchainMetablock(Thread):
    ...
    def run(self):
        time_redis = redis.time()
        print("Thread started for metablock ", self.body['key'], "currently time in redis: ", time_redis)
        tx_hash = store_var_contract.functions.addMetablock(self.body['key'], self.body['creation_date'], self.body['block_type'], self.body['checksum'], self.body['providers'], self.body['size']).transact()
        receipt = wait_for_receipt(w3, tx_hash, 1)
        time_redis = redis.time()
