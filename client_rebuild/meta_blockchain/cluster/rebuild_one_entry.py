...
class TransactionRebuildMetadata(Thread):
    ...
    def run(self):
        ...
        if(self.body[0] == "f"):
          #Rebuild a metadoc
          key = store_var_contract.functions.getMetadocMap(self.body[6:]).call()
          #[0] = date of the creation
          #[1] = list of blocks
          #[2] = original size
          #[3] = entangling_blocks
          print("Searching metadata in blockchain for path_given (metadoc) : "+self.body[6:])
          if(key[2] == 0):
          ...
          #Rebuild a metablock
          metablock = store_var_contract.functions.getMetablockMap(self.body[7:]).call()
          ...
          #Add metadoc in Redis database
          redis.hmset("blocks:{:s}".format(self.body[7:]), metablock_to_redis)
          print(metablock_to_redis)
      
        time_redis = redis.time()

        link="http://cluster_stats:80/api/receive_rebuild.php?request="+self.body+"///"+str(time_redis[0])+"."+str(time_redis[1])+"///All"
        requests.get(link)
        print("\nThread finished for: "+self.body+" at "+str(time_redis[0])+"."+str(time_redis[1]))

        #Write stats in file
        fichier = open("save_check_corruption.txt", "a")
        fichier.write(self.body+" "+str(time_redis[0])+"."+str(time_redis[1])+"\n")
        fichier.close()
